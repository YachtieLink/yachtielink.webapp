...
**2026-04-03** — Rally 009 Session 6, Lane 3 (feat/experience-transfer): Endorsement dormancy system. New `lib/endorsements/visibility.ts`: `recalculateEndorsementDormancy(userId)` — fetches user's yachts post-transfer, gets endorsements where user is endorser or recipient, hides endorsements when endorser+recipient no longer share any active yacht (`is_dormant = true`), unhides when they do. Uses service-role client to bypass RLS. New `lib/network/colleague-rebuild.ts`: `rebuildColleagueConnections()`. Applied `is_dormant` filter (`.or('is_dormant.is.null,is_dormant.eq.false')`) to all 12 endorsement display queries across the app: profile, network, public profile, CV, endorsement API, pin API. Pinning dormant endorsements now returns 400. Existing endorsements with `is_dormant = null` remain visible (null treated as non-dormant).
