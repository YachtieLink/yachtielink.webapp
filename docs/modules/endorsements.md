...
**2026-04-02** — Worktree lane 1 (pending push): ghost_endorser join added to `getProfileSections` + `getCvSections` — private dashboard and owner CV preview now show ghost endorser names. Name-resolution chain updated in `EndorsementsSection.tsx` (private) and `CvPreview.tsx`. Open: stale inline endorsements query in `app/(protected)/app/cv/preview/page.tsx` still lacks ghost join (pre-existing, captured in backlog).
