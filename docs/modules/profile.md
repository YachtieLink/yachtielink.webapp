...
**2026-04-04** — feat/per-context-focal-zoom bug fix pass: `app/(protected)/app/profile/photos/page.tsx` — `|| 50` → `?? 50` for all focal fallbacks (falsy-zero fix), `isProFromRecord()` replaces bare `=== 'pro'` (expiry check), hero context thumbnail changed to `max-w-[160px] aspect-[2/1]` (responsive fix), thumbnail row gets `justify-center`. `app/(protected)/app/land-experience/[id]/edit/page.tsx` — IDOR fix (userId state + `.eq('user_id')` on all ops), maxLength=200, session-expired toast. `app/(protected)/app/land-experience/new/page.tsx` — session-expired toast, maxLength=200. `lib/queries/profile.ts` `getExtendedProfileSections` now selects all 17 user_photos columns to match `ProfilePhoto` type.
