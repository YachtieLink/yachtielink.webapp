...
### Abuse escalation protocol (yl_phase1_actionables.json)
...
| Topic | Document |
|-------|----------|
| Existing abuse escalation | `yl_phase1_actionables.json` → `abuse_escalation_protocol` |
| Rate limiting infrastructure | `yl_tech_stack.md` → Vercel KV |
| RLS enforcement | `yl_security.md` → Section 1.2 |
| Endorsement gating rule | `yl_decisions.json` → D-009 |
| Retraction visibility (backend only) | `yl_decisions.json` → D-005 |
| Database schema | `yl_schema.md` |
