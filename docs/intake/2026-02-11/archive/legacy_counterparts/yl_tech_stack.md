...
| **Real-time Messaging** | Supabase Realtime | Free tier | Free | Free (migrate if >200 concurrent) | WebSocket messaging between connections |
...
WebSocket connection events tracked in PostHog:
- `messaging.connection.opened`
- `messaging.connection.closed`

Dashboard monitors concurrent connections. Tripwire at 150 concurrent (75% of soft limit) triggers migration planning.
...
|-----------|---------|-------------------|---------|-----------|
| Messaging | Supabase Realtime | Cloudflare Durable Objects | >200 concurrent connections | ~â‚¬15-20/mo |
...
**Full security protocols:** See `yl_security.md` for external attack defense, system abuse prevention, resource exploitation limits, and code review checklist.
...
| `messaging.connection.opened` | WebSocket monitoring | On connect |
| `messaging.connection.closed` | WebSocket monitoring | On disconnect |
