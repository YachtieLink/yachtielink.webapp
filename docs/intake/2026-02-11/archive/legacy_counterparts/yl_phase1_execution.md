...
- Connections alone do not permit endorsements
...
- Users can connect without shared employment
- Connections require acceptance
- Messaging allowed between connections and verified co-workers
- No public graph, no discovery, no ranking, no trust impact

### Excluded Features
- Group chats
- Broadcast to non-connections
...
**Reference:** See `yl_decisions.json` → D-027
...
- Both parties see graph proximity: mutual connections, shared yachts, endorsement paths
...
**Reference:** See `yl_decisions.json` → D-022
...
**Reference:** See `yl_decisions.json` → D-024, D-026
...
**Reference:** See `yl_decisions.json` → D-025
...
**Reference:** See `yl_phase1_actionables.json` → `metrics_and_tripwires`
...
**Reference:** See `yl_phase1_actionables.json` → `growth_controls`
...
**Reference:** See `yl_phase1_actionables.json` → `abuse_escalation_protocol`
