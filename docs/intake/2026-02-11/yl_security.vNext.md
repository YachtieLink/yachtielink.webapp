...
| `EXISTS (SELECT 1 FROM contacts...)` | User has relationship | Messaging access |
...
System abuse triggers feed into the escalation protocol defined in `yl_phase1_actionables.vNext.json`:
...
### 2.5 Timeline Abuse Vectors (vNext)

| Abuse vector | Risk | Mitigation |
|-------------|------|------------|
| Doxxing via posts | Personal info exposure | Report flow + takedown, content scanning rules, educate users |
| Stalking via location hints | Real-world tracking | Location hints are coarse, optional, and non-searchable |
| Coercion to join encounters | Abuse of IRL confirmation | Right of exit, report flow, rate limits on encounters |
| Non-consensual imagery | Safety and legal risk | Immediate hide on report, takedown, escalation |
...
| Post create | TBD | TBD | Per user |
| Post media upload | TBD | TBD | Per user |
| Post comment create | TBD | TBD | Per user |
| Interaction create | TBD | TBD | Per user |
| Interaction media upload | TBD | TBD | Per user |
| Interaction comment create | TBD | TBD | Per user |
...
| Post create/edit/delete | Yes | 90 days |
| Interaction create/confirm/exit | Yes | Indefinite |
| Content reports and takedowns | Yes | Indefinite |
...
| Topic | Document |
|-------|----------|
| Abuse escalation tiers | `yl_phase1_actionables.vNext.json` → `abuse_escalation_protocol` |
| Rate limit infrastructure | `yl_tech_stack.vNext.md` → Vercel KV |
| Growth pause trigger | `yl_phase1_actionables.vNext.json` → `growth_controls` |
| Trust outcome protection | `yl_principles.md` → Canonical monetisation rule |
| WebSocket session limits | `yl_phase1_actionables.vNext.json` → `websocket_concurrent_sessions` |
