-- Fix: Don't let builder/length boosts outrank perfect name matches
-- When base_sim = 1.0 (exact name match), skip boosts entirely.
-- This prevents "TS Eclipse Star 3" (27m, sim=0.89+0.15=1.04) from
-- ranking above "TS Eclipse Star" (90m, sim=1.0) when the parser
-- extracted the wrong length.
...
BEGIN
  ...
  SELECT
    ...
    -- Perfect name matches should never be outranked
    (bs.base_sim
     + CASE
