      ...
      <div className="flex flex-wrap gap-1.5 flex-1">
        {shown.map((cert) => {
          ...
          return (
            <span
              key={cert.id}
              className="text-xs px-2.5 py-1 rounded-full bg-white/40 text-[var(--color-text-primary)] border border-[var(--color-border-subtle)] border border-[var(--color-border-subtle)]"
            >
              {name}
            </span>
          )
